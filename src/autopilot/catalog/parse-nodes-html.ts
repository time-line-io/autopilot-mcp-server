...
import { evalAst, extractStringLiteral, isRegisterTypeCallee } from "./ast-eval.js";
...
function extractScriptsAndDocs(moduleHtml: string) {
	...
	$("script").each((_, el) => {
		...
		if (!helpName && !templateName && (typeAttr === "text/javascript" || typeAttr === "")) {
...
function parseEditorJsForRegisterTypes(jsSource: string) {
	...
	const wrappers = new Map<string, { params: string[]; typeArg: any; optionsArg: any }>();

	function maybeRegisterWrapper(fnName: string, params: string[], bodyNode: any) {
	...
	(walk as any).simple(ast, {
		CallExpression(node: any) {
			...
			if (node.callee?.type === "Identifier" && wrappers.has(node.callee.name)) {
				...
				const optionsSource = jsSource.slice(w.optionsArg.start, w.optionsArg.end);
...
export function parseNodesHtml(htmlString: string, { verbose = false } = {}) {
	...
	for (;;) {
		...
		const { js, helpByType, templateByType } = extractScriptsAndDocs(moduleHtml);
		...
		for (const jsSource of js) {
			const { found, warnings: jsWarnings } = parseEditorJsForRegisterTypes(jsSource);
