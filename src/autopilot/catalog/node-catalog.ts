...
function defaultCustomModules() {
  return [
    "@time-line-autopilot/node-red-nodes",
    "node-red-dashboard-2-time-line-autopilot",
  ];
}
...
export class NodeCatalog {
  ...
  constructor(config: any) {
    ...
    this._verbose =
      !!this._config.verbose || truthy(env.NODE_MCP_VERBOSE_CATALOG);
  ...
  private _markCustom(node: any) {
    ...
    const isCustom =
      ...
      (module &&
        Array.from(this._customModules).some((m) => module.startsWith(m)));
  ...
  async search({
    ...
    const c = await this.getCatalog();
    const nodes = customOnly
      ? c.nodes.filter((n: any) => n.isCustom)
      : c.nodes.slice();
