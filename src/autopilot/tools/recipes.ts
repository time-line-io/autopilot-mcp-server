...
function parseOscAddress(intent: string) {
  ...
  if (/\bkeypoint\b/i.test(s) && /\bprogress\b/i.test(s))
    return "/keypoint/progress";
...
function wantsSongDetailsWidget(intent: string) {
  ...
  return (
    s.includes("song details") ||
    s.includes("now playing") ||
    s.includes("playhead")
  );
}
...
function wantsKeypointDetailsWidget(intent: string) {
  const s = String(intent || "").toLowerCase();
  return (
    s.includes("keypoint details") || s.includes("current keypoint widget")
  );
}
...
function wantsUpcomingKeypointsWidget(intent: string) {
  const s = String(intent || "").toLowerCase();
  return (
    s.includes("upcoming keypoints") || s.includes("next keypoints widget")
  );
}
...
function pickValueMapping(intent: string, triggerType: string) {
  ...
  if (wantsBpm(intent)) {
    ...
    const expr =
      triggerType === "tl-beat" ? "msg.payload.bpm" : "msg.payload.bpm";
...
export default function registerRecipeTools(
  server: McpServer,
  ...
  server.tool(
    ...
    async ({ intent, customOnly }) => {
      ...
      const timing = (() => {
        if (triggerType === "tl-beat")
          return { mode: "stream", output: "output" };
        if (triggerType === "tl-current-track")
      ...
      if (wantsSongDetailsWidget(intent)) {
        steps.push({
          ...
          role: "dashboard",
          notes:
            "Wire a TL keypoint node’s *stream* output (or any msg.payload currentTrack snapshot) into this widget.",
        });
      }
      if (wantsKeypointDetailsWidget(intent)) {
        steps.push({
          ...
          role: "dashboard",
          notes:
            "Wire any tl-keypoint-* node’s *event* output into this widget (it listens to enter/exit edges).",
        });
      }
      if (wantsUpcomingKeypointsWidget(intent)) {
        steps.push({
          ...
          role: "dashboard",
          notes:
            "Wire tl-current-track output (or any msg.payload currentTrack snapshot) into this widget. It uses payload.keypoints and payload.currentPositionMs.",
        });
      }
      ...
      if (
        (timing as any).mode === "event" &&
        triggerType.startsWith("tl-keypoint")
      ) {
        steps.push({
          step: String(steps.length + 1),
          node: "switch",
      ...
      if (protocol === "osc") {
        ...
        steps.push({
          ...
          role: "sink",
          notes:
            "Wire into tl-output. Leave Address blank to use msg.topic as OSC address, or set Address to a fixed address.",
          config: {
        ...
        steps.push({
          ...
          role: "config",
          notes:
            "Create/reuse a MIDI server config: set protocol=midi and pick midiPort. Then send msg.payload as raw bytes (e.g. [144,60,127]) or msg.midi (type/channel/data).",
        ...
        steps.push({
          ...
          role: "sink",
          notes:
            "Wire into tl-output. For MIDI, msg.topic is ignored; msg.payload must be bytes/Buffer or msg.midi object.",
          config: {
            server: "(select your tl-output-server)",
            vizType: valueMapping.vizType || "led",
          },
