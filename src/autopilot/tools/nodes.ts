...
export default function registerNodeTools(server: McpServer, config: AutopilotMcpConfig) {
	...
	server.tool(
		...
		"Searches installed node documentation (help text, defaults, metadata). Best for queries like 'group progress', 'OSC', 'keypoint'.",
		{
			...
			module: z.string().optional().describe("Optional module filter substring"),
			category: z.string().optional().describe("Optional category filter substring"),
	...
	server.tool(
		...
		async ({ module, set }) => {
			const info = await callNodeRed("get", `/nodes/${module}/${set}`, null, config);
			return {
				content: [{ type: "text", text: JSON.stringify(info, null, 2) }],
			};
		},
	...
	server.tool(
		...
		async ({ module, set, enabled }) => {
			try {
				await callNodeRed("put", `/nodes/${module}/${set}`, { enabled }, config);
				return {
					content: [
						{
							type: "text",
